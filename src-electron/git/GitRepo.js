...
class GitRepo {
    ...
    async commit (message, options) {
        ...
        assert(name, 'git commit user is empty')
        assert(email, 'git commit email is empty')

        const author = nodegit.Signature.create(name, email, Date.now(), new Date().getTimezoneOffset())
    ...
    async checkout (branchName) {
        ...
        await this.rawRepo.checkoutBranch(branchName)
    }
